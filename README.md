...
training run under `reports/figures/` (gitignored, regenerated each run).
The two above are static copies committed to `assets/` specifically so they
render on GitHub — after a new run, refresh them with:

```bash
cp reports/figures/confusion_matrix.png reports/figures/roc_curve.png assets/
```
